...
export const UserProfile: React.FC<UserProfileProps> = ({
  ...
  const loadStreakData = async (activities: any[], username: string) => {
    ...
    try {
      ...
      const streakPromises = activities.slice(0, 10).map(async (activity) => {
        // Use activityId directly since that's what we have in the activity type
        const activityIdentifier = activity.activityId;
        ...
        try {
          ...
          const streak = await fetchActivityStreak(username.toLowerCase(), numericActivityId);
  ...
  const fetchUserProfile = async (usernameToFetch: string) => {
    ...
    try {
      const data = await fetchSneakerDBProfile(usernameToFetch.toLowerCase());
      ...
      if (data?.activities) {
        await loadStreakDataWithRetry(data.activities, usernameToFetch.toLowerCase());
      }
    } catch (err) {
